...
namespace Inmobiliaria_.Net_Core.Repositorios
{
    ...
    {
        public Repositorio_PagoMySQL(IConfiguration configuration)
            : base(configuration)
        {
        }
            ...
            using (var connection = new MySqlConnection(connectionString))
            {
                ...
                using (var command = new MySqlCommand(sql, connection))
                {
                    ...
                    command.Parameters.AddWithValue(
                        "@fecha_anulacion",
                        pago.Fecha_Anulacion.HasValue
                            ? pago.Fecha_Anulacion.Value
                            : DBNull.Value
                    );
                    ...
                    command.Parameters.AddWithValue(
                        "@id_usuario_creador",
                        pago.ID_Usuario_Creador.HasValue
                            ? pago.ID_Usuario_Creador.Value
                            : DBNull.Value
                    );

                    command.Parameters.AddWithValue(
                        "@id_usuario_finalizador",
                        pago.ID_Usuario_Finalizador.HasValue
                            ? pago.ID_Usuario_Finalizador.Value
                            : DBNull.Value
                    );
    ...
    return pago;
        ...
        private Pago MapearPago(MySqlDataReader reader)
        {
            return new Pago
            {
                ...
                Fecha_Anulacion =
                    reader.IsDBNull(reader.GetOrdinal("fecha_anulacion"))
                        ? null
                        : reader.GetDateTime("fecha_anulacion"),
                ...
                ID_Usuario_Creador =
                    reader.IsDBNull(reader.GetOrdinal("id_usuario_creador"))
                        ? null
                        : reader.GetInt32("id_usuario_creador"),
